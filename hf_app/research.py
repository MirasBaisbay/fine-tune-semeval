...
class MediaProfiler:
    ...
    def _lookup_freedom_rating(self, headquarters: str | None) -> dict:
        """
        Look up RSF Press Freedom Index score for the outlet's country.
        ...
        """
        from config import FREEDOM_INDEX_FILE, FREEDOM_LABELS, COUNTRY_NAME_ALIASES
        ...
        if not os.path.exists(csv_path):
            logger.warning(f"Freedom index file not found: {csv_path}")
            return result
        ...
        try:
            with open(csv_path, "r", encoding="utf-8", errors="replace") as f:
                ...
                for row in reader:
                    ...
                    if row_country.lower() == country_name.lower():
                        # Parse score (uses comma as decimal separator)
                        ...
                        result["freedom_score"] = round(score, 2)
                        result["freedom_rank"] = rank
            ...
            logger.error(f"Failed to load freedom index: {e}")
    ...
    def profile(
        ...
        report = ComprehensiveReportData(
            ...
            # Research results — ownership (sanitize LLM "null" strings)
            ...
            # External analyses (filter out MBFC — we use MBFC methodology, not their ratings)
            external_analyses=[
            ...
            **self._lookup_freedom_rating(self._sanitize_null(ownership.headquarters)),

            # Articles index (for linking source references in UI)
            articles_index=[
