...
if "report_data" in st.session_state and st.session_state.get("report_data"):
    ...
    with col5:
        freedom_label = rd.get("freedom_label", "—")
        freedom_score = rd.get("freedom_score")
        freedom_rank = rd.get("freedom_rank")
        freedom_delta = f'Score: {freedom_score}' + (f' | Rank: #{freedom_rank}' if freedom_rank else '') if freedom_score else "—"
    ...
    **What gets analyzed:**
    ...
    - **Country Freedom** — RSF Press Freedom Index for the outlet's country
    - **External Analyses** — Ad Fontes, NewsGuard, academic reviews
