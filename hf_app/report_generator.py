...
COUNTRY FREEDOM (RSF Press Freedom Index):
- Country: {getattr(data, 'country', None) or 'Unknown'}
- Freedom Score: {getattr(data, 'freedom_score', None) or 'N/A'}/100
- Freedom Label: {getattr(data, 'freedom_label', None) or 'N/A'}
- Global Rank: #{getattr(data, 'freedom_rank', None) or 'N/A'}
...
3. **Detailed Report**: A list of metrics.
   ...
   - Country Freedom: {getattr(data, 'freedom_label', 'N/A') or 'N/A'} ({getattr(data, 'freedom_score', 'N/A') or 'N/A'}/100)
...
6. **Analysis / Bias**:
   ...
   - Mention external critiques found in the 'external_analyses' data. Note: MBFC is our methodology source, not an external analysis — do not cite MBFC as an external review.
